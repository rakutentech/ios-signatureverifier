...
class KeyStoreSpec: QuickSpec {
    override func spec() {
        ...
        describe("KeyStore") {
            let keyStore = KeyStore(account: "https.endpoint.com.keys", service: "unit-tests")
            ...
            context("when there is another keystore created with different account") {
                let keyStore2 = KeyStore(account: "https.endpoint.com.v2.keys", service: "unit-tests")
                beforeEach {
                    keyStore2.empty()
                }
            ...
            context("when there is another keystore created with the same account") {
                let keyStore2 = KeyStore(account: "https.endpoint.com.keys", service: "unit-tests")
